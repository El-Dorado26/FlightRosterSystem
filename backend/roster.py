...
async def generate_roster(
    ...
    """
    Generate a complete roster for a flight.
    
    This endpoint:
    1. Validates the flight exists
    2. Selects crew automatically or uses manual selection
    3. Assigns seats to passengers without seat numbers
    4. Creates a complete roster snapshot
    5. Stores it in the database
    """
    # Get flight with all relationships
    ...
    # STEP 1: Select Flight Crew
    if roster_create.auto_select_crew:
        # Automatic crew selection using greedy approach
    ...
    # STEP 2: Select Cabin Crew
    if roster_create.auto_select_crew:
        # Automatic cabin crew selection
        ...
        # Assign manually selected cabin crew to flight
        for crew in cabin_crew_members:
            crew.flight_id = roster_create.flight_id
    
    # STEP 3: Validate crew selection
    ...
    # STEP 4: Get passengers and assign seats if needed
...
@router.get("/", response_model=List[schemas.RosterListResponse])
async def list_rosters(
    ...
    """
    List all rosters with optional filters.
    """
    ...
    rosters = query.order_by(models.Roster.generated_at.desc()).all()
    return rosters


@router.get("/{roster_id}", response_model=schemas.RosterResponse)
async def get_roster(roster_id: int, db: Session = Depends(get_db)):
    """
    Retrieve a specific roster by ID.
    """
...
async def download_roster_json(roster_id: int, db: Session = Depends(get_db)):
    ...
    filename = f"roster_{roster.roster_name.replace(' ', '')}{roster.id}.json"
